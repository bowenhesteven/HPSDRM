...
while (i < n_realizations) {
  ...
  output <- list(car_simu = car_simu,
                 ...
                 par_draws = par_draws)
  i = i + 1
  car.simu.res.gaus.sd0.1[[i]] <- output
}
saveRDS(car.simu.res.gaus.sd0.1, file = (paste0(proj_root, "/data/gen/sensitivity_analysis/car.simu.res.gaus.sd0.1.rds")))
