...
priors <- list(priormean_intercept = beta0,
               ...
               prior_logrho_gaussian_sd = 0.1,
               prior_logsigma_gaussian_mean = log_sigma,
               prior_logsigma_gaussian_sd = 0.1,
               prior_rho_min = 1.3,
               prior_sigma_max = 1,
               prior_logit_car_rho_mean = logit_car_rho,
               prior_logit_car_rho_sd = 0.1,
               prior_car_gamma_shape = car_prec,
               prior_car_gamma_scale = 1)

i <- 0
field.car.simu.res.sd0.1 <- list()
n_realizations <- 100
while (i < n_realizations) {
  ...
  i = i + 1
  field.car.simu.res.sd0.1[[i]] <- field.car.simu.output
}
saveRDS(field.car.simu.res.sd0.1, file = (paste0(proj_root, "/data/gen/sensitivity_analysis/field.car.simu.res.sd0.1.rds")))
