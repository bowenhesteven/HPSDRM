...
i <- 0
car.simu.res.gaus.sd2.shape.1.39.scale.0.85 <- list()
while (i < n_realizations) {
  ...
  output <- list(car_simu = car_simu,
                 ...
                 par_draws = par_draws)
  i = i + 1
  car.simu.res.gaus.sd2.shape.1.39.scale.0.85[[i]] <- output
}
saveRDS(car.simu.res.gaus.sd2.shape.1.39.scale.0.85, file = (paste0(proj_root, "/data/gen/sensitivity_analysis/car.simu.res.gaus.sd2.shape.1.39.scale.0.85.rds")))
