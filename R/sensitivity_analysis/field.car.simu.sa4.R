...
priors <- list(priormean_intercept = beta0,
               ...
               prior_logit_car_rho_sd = 2,
               prior_car_gamma_shape = prior_gamma_shape,
               prior_car_gamma_scale = prior_gamma_scale)

i <- 0
field.car.simu.res.sd1.logitlambda.sd2.shape.1.39.scale.0.85 <- list()
n_realizations <- 100
while (i < n_realizations) {
  ...
  i = i + 1
  field.car.simu.res.sd1.logitlambda.sd2.shape.1.39.scale.0.85[[i]] <- field.car.simu.output
}
saveRDS(field.car.simu.res.sd1.logitlambda.sd2.shape.1.39.scale.0.85, file = (paste0(proj_root, "/data/gen/sensitivity_analysis/field.car.simu.res.sd1.logitlambda.sd2.shape.1.39.scale.0.85.rds")))
